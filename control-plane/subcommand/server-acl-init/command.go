...
func (c *Command) Run(args []string) int {
	...
	if c.flagClient {
		...
		err = c.createACLPolicyRoleAndBindingRule("client", agentRules, consulDC, primaryDC, localPolicy, primary, localComponentAuthMethodName, serviceAccountName, consulClient)
		if err != nil {
			c.log.Error(err.Error())
			return 1
		}
