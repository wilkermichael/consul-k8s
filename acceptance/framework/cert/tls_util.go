...
// GenerateRootCA generates a CA with the provided
...
func GenerateRootCA(commonName string) (
...
func GenerateIntermediateCA(commonName string, rootCert *x509.Certificate, rootSigner crypto.Signer) (
	crypto.Signer,
	string,
	string,
	*x509.Certificate,
	...
	signer, keyPem, err := privateKey()
	if err != nil {
		return signer, keyPem, caCertPem, caCertTemplate, err
	}
	...
	bs, err := x509.CreateCertificate(
		rand.Reader, caCertTemplate, rootCert, rootSigner.Public(), rootSigner)
	if err != nil {
		return signer, keyPem, caCertPem, caCertTemplate, err
	}
