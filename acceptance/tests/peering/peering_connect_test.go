	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			commonHelmValues := map[string]string{
				"global.imageK8S":        "ndhanushkodi/consul-k8s-dev:peering38",
				"global.image":           "ndhanushkodi/consul-dev:peering12",
			...
			//helpers.MergeMaps(commonHelmValues, serverHelmValues)

			// Install the consul cluster with servers in the default kubernetes context.
			peer1ConsulCluster := consul.NewHelmCluster(t, commonHelmValues, serverClusterContext, cfg, releaseName)
			peer1ConsulCluster.Create(t)
			...
			peer2ConsulCluster := consul.NewHelmCluster(t, clientHelmValues, clientClusterContext, cfg, releaseName)
			peer2ConsulCluster.Create(t)
			...
			consulClient, _ := peer1ConsulCluster.SetupConsulClient(t, c.ACLsAndAutoEncryptEnabled)
			...
			serverQueryClientOpts := &api.QueryOptions{Namespace: staticServerNamespace, Partition: defaultPartition}
			clientQueryClientOpts := &api.QueryOptions{Namespace: StaticClientNamespace, Partition: defaultPartition}
			//
			//if !c.mirrorK8S {
